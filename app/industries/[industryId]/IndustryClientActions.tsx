...
export default function IndustryClientActions({ industryName }: { industryName: string }) {
  const { openQuoteModal } = useModals();

  return (
    <div className="pt-2 flex flex-wrap items-center gap-3">
      <button
        onClick={() => openQuoteModal(null, `Inquiry for ${industryName} wear solutions`)}
      ...
      >
        ...
        <span>Request {industryName} RFQ</span>
      </button>
      ...
      >
        ...
        <span>Direct Dispatch: {COMPANY_INFO.phoneFormatted}</span>
      </a>
    </div>
  );
}
